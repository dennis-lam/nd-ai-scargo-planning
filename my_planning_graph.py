...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Obtain current literal level
        s_level = self.s_levels[level]
        # Initialize action level
        a_level = set()
        # Handle each action.
        for action in self.all_actions:
            # Create a new instance of action node.
            ...
            if a_node.prenodes.issubset(s_level):
                ...
                # Add action node to current level.
                a_level.add(a_node)
        # Add current level.
    ...
    def add_literal_level(self, level):
        ...
        # Obtain last action level.
        a_level = self.a_levels[level - 1]
        # Initialize literal level
        s_level = set()
        # Handle each action
        for a_node in a_level:
            # Handle each effect node
            for effnode in a_node.effnodes:
                # Create a new instance of literal node.
                s_node = PgNode_s(effnode.symbol, effnode.is_pos)
                # Connect nodes
                s_node.parents.add(a_node)
                a_node.children.add(s_node)
                # Add literal node to current level
                s_level.add(s_node)
        # Add current level.
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Handle each combination of effect nodes
        for effnode_a1 in node_a1.effnodes:
            for effnode_a2 in node_a2.effnodes:
                # symbol and is_pos are created by PgNode_a.action.effect_add and PgNode_a.action.effect_rem
                # Check any same symbol but opposite effect
                if effnode_a1.symbol == effnode_a2.symbol and effnode_a1.is_pos != effnode_a2.is_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        def contains_interference(prenodes, effnodes):
            """
            Check any interference found in given precondition nodes and effect nodes
            """
            for prenode in prenodes:
                for effnode in effnodes:
                    # Check any same symbol but opposite effect
                    if prenode.symbol == effnode.symbol and prenode.is_pos != effnode.is_pos:
                        return True
            return False

        # No need to check remaining case when contains_interference() returns true
        if contains_interference(node_a1.prenodes, node_a2.effnodes):
            return True
        else:
            return contains_interference(node_a2.prenodes, node_a1.effnodes)
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # Reference : Udacity forum #227344
        # Check whether any parents are mutually exclusive
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # Each each parent action nodes combination
        for node_a1 in node_s1.parents:
            for node_a2 in node_s2.parents:
                # Return false if any pairs of action are not mutex
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        def get_level(fluent):
            """
            Obtain given fluent level
            """
            # Check each level
            for index, level in enumerate(self.s_levels):
                # Return level index when given fluent is found.
                for node_s in level:
                    if node_s.is_pos and node_s.symbol == fluent:
                        return index

            # Return self-defined maximum value if no valid levels found
            return 99999

        # Sum up each goal fluent level.
        return sum([get_level(fluent) for fluent in self.problem.goal])
